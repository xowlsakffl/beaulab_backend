...
- 공통 운영 인프라
  ...
  - Codex AI를 적극 활용하여 반복 구현, 리팩토링, 보일러플레이트 생성 및 구조 검증 효율화
  - AI 기반 개발 워크플로우를 활용해 생산성과 유지보수성을 동시에 고려한 개발 프로세스 구축
...
- 관리자 화면 처리 이력은 공통 `OperationHistory` 구조로 기록합니다.
...
| `AdminNote` | 운영자 메모 |
| `OperationHistory` | 관리자 화면용 운영 처리 이력과 변경 상세 |
...
- 운영 히스토리
  - `operation_histories`는 대상/수행자/action/reason을 저장합니다.
  - `operation_history_changes`는 변경 필드별 전/후 값을 저장합니다.
  - 단건 변경과 다중 변경 모두 같은 구조를 사용합니다.
