...
final class HospitalEventUpdateHistoryRecordAction
{
    ...
    public function recordUpdated(HospitalEvent $event, array $before): void
    {
        ...
        foreach (OperationHistoryChangeSetBuilder::groupedFromSnapshots($before, $this->snapshot($event), ['allow_status', 'hospital_status', 'admin_status']) as $action => $changes) {
            $this->record($event, $action, 'staff.hospital_event.update', $changes);
        }
    }
     ...
     * @return array<string, array{label:string,value:mixed,display:?string}>
     */
    public function capturePeriod(HospitalEvent $event): array
    {
        return [
     ...
     * @param  array<string, array{label:string,value:mixed,display:?string}>  $before
     */
    public function recordPeriodUpdated(HospitalEvent $event, array $before): void
    {
        $this->record(
            event: $event,
            action: OperationHistory::ACTION_UPDATED,
            source: 'staff.hospital_event.period',
            changes: OperationHistoryChangeSetBuilder::fromSnapshots($before, $this->capturePeriod($event)),
        );
    }
    ...
    private function snapshot(HospitalEvent $event): array
    {
        return [
            ...
            'manager_staff' => $this->item('담당자', $event->manager_staff_id ? (int) $event->manager_staff_id : null, $event->managerStaff?->name),
            'event_type' => $this->item('등록 유형', $event->event_type, $event->event_type === HospitalEvent::TYPE_TEXT ? '텍스트 등록' : '이미지 등록'),
            'allow_status' => $this->item('검수상태', $event->allow_status, HospitalEvent::allowStatusLabel((string) $event->allow_status)),
            'hospital_status' => $this->item('공개여부', $event->hospital_status, HospitalEvent::hospitalStatusLabel((string) $event->hospital_status)),
            'admin_status' => $this->item('강제중지', $event->admin_status, HospitalEvent::adminStatusLabel((string) $event->admin_status)),
            ...
            'consultation_price' => $this->item('상담신청단가', (int) $event->consultation_price, number_format((int) $event->consultation_price).'P'),
            'options' => $this->item('이벤트 옵션', $this->optionValue($event), $this->optionDisplay($event)),
            'procedure_targets' => $this->item('시술 대상', $event->procedure_targets ?? [], OperationHistoryDisplayValue::lines($event->procedure_targets ?? [])),
            'procedure_benefits' => $this->item('시술 장점', $event->procedure_benefits ?? [], OperationHistoryDisplayValue::lines($event->procedure_benefits ?? [])),
            'side_effect_notice' => $this->item('부작용안내', $event->side_effect_notice, $event->side_effect_notice),
            'thumbnail_image' => $this->item('썸네일', $event->thumbnailImage?->path, OperationHistoryDisplayValue::fileName($event->thumbnailImage?->path)),
            'event_page_image' => $this->item('이벤트 페이지', $event->eventPageImage?->path, OperationHistoryDisplayValue::fileName($event->eventPageImage?->path)),
    ...
    private function beforeAfterPhotosDisplay(HospitalEvent $event): string
    {
        ...
        return $event->beforeAfterPhotos->groupBy('sort_order')->values()
            ->map(fn ($photos, int $index): string => ($index + 1)."세트\n전: "
                .OperationHistoryDisplayValue::fileName($photos->firstWhere('collection', HospitalEvent::COLLECTION_BEFORE_PHOTO)?->path)
                ."\n후: ".OperationHistoryDisplayValue::fileName($photos->firstWhere('collection', HospitalEvent::COLLECTION_AFTER_PHOTO)?->path))
            ->implode("\n\n");
    }
    ...
    {
        return OperationHistoryDisplayValue::lines(collect($this->categoryValue($event))
            ->map(static fn (array $category): string => ($category['is_primary'] ? '[대표] ' : '').$category['path'])
            ->all());
    }

    /**
     * @return array<int, array{id:int,name:string,is_career_visible:bool,is_activity_visible:bool,sort_order:int}>
     */
    ...
    {
        return OperationHistoryDisplayValue::lines(collect($this->doctorValue($event))
    ...
    {
        return OperationHistoryDisplayValue::lines(collect($this->optionValue($event))
