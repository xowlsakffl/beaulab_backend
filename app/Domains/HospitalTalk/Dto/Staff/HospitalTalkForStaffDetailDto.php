...
final readonly class HospitalTalkForStaffDetailDto
{
    public function __construct(public array $talk) {}

    public static function fromModel(HospitalTalk $talk, bool $includeComments = false): self
    {
        $data = [
            'id' => (int) $talk->id,
        ...
        if ($includeComments) {
            $data['comments'] = self::resolveComments($talk)
                ->map(fn (HospitalTalkComment $comment): array => [
                    'id' => (int) $comment->id,
     ...
     */
    private static function resolveCategories(HospitalTalk $talk): Collection
    {
     ...
     */
    private static function resolveImages(HospitalTalk $talk): Collection
    {
     ...
     */
    private static function resolveAdminNotes(HospitalTalk $talk): Collection
    {
     ...
     */
    private static function resolveComments(HospitalTalk $talk): Collection
    {
