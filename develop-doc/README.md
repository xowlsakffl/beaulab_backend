...
- Staff API는 대시보드, 병원/입점신청/뷰티/회원/의료진/전문가, 카테고리/해시태그, 병원 이벤트/DB/리얼모델 DB, 동영상, 토크/후기/평가/신고, 공지/FAQ 운영을 담당한다.
- Hospital API는 현재 병원 계정 인증/프로필/비밀번호/관리자 메모와 병원 동영상 요청 생성/파트너 취소를 제공한다.
