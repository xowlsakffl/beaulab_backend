...
- [백엔드 보완 및 배포 점검](./backend-hardening.md)
...
- Queue 표준 런타임은 Redis + Horizon이며, 현재 실제 사용 큐는 비밀번호 재설정/병의원 계정 초대 메일(`mail`), Push 발송(`notifications`), 공통 문자 발송(`sms`), 채팅/인앱 브로드캐스트(`chat`)이다.
