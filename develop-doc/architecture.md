...
- API(`/admin/api/*`)는 **관리자 페이지와 동일한 세션 기반 인증**을 사용한다.
- 따라서 관리자 API는 **`api` 미들웨어 그룹을 사용하지 않고**,  
  반드시 `web + auth:admin` 미들웨어를 사용한다.
- `/admin/api/*`는 “API처럼 보이는 JSON 엔드포인트”일 뿐,  
  **stateless API가 아니다.**
---
...
- 보호 API: `/admin/api/*` + `web` + `auth:admin`
