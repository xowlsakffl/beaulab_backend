...
| `ContentReportItem` | `content_report_items` | 신고 1건에 포함된 실제 대상 항목 |
| `ContentReportState` | `content_report_states` | 신고 대상별 현재 신고 상태와 집계 |

`ContentReport`는 신고 1건의 공통 정보와 대표 대상(`target_type + target_id`)을 저장한다. `ContentReportItem`은 그 신고 안에 실제로 포함된 대상 항목을 저장한다. 일반 게시글/댓글/평가 신고는 item 1건이고, 채팅 메시지 신고는 한 번에 최대 5개 item을 저장한다.

`ContentReportState`는 대표 대상 `target_type + target_id`별로 1건만 유지하며, Staff 신고게시물 리스트와 상세 화면은 이 상태 테이블을 기준으로 조회한다.

중복 신고 제한은 `content_report_items`의 `reporter_user_id + target_type + target_id` unique 기준으로 처리한다. 따라서 채팅 메시지 묶음 신고에서 대표 메시지가 달라도, 이미 신고한 메시지가 하나라도 포함되면 재신고를 거부한다.
...
1. 신고 item 기준으로 같은 신고자가 이미 신고한 대상인지 확인한다.
2. 신고 로그를 `content_reports`에 생성한다.
...
- 이미 신고자가 차단한 사용자 메시지는 추가 신고할 수 없다.
...
채팅 메시지 신고의 `ContentReport.target_id`는 요청 메시지 중 첫 번째 메시지를 대표 대상으로 사용한다. 실제 신고된 최대 5개 메시지는 `ContentReportItem`으로 저장한다.
...
경고/무시는 신고 대상 작성자 유저에게 적용되는 공통 제재다. 게시글/댓글/후기/평가는 `ADMIN_HIDDEN` 상태에서만 가능하고, 채팅 메시지는 `INVALID` 상태에서만 가능하다.

`WARNED`로 바꾸면 대상 작성자의 `AccountUser.warning_count`가 1 증가한다. 누적 경고가 10건 이상이면 사용자는 `BLOCKED` 상태가 된다. `WARNED`에서 `IGNORED`로 변경하면 경고 수가 1 감소하고, 경고 수가 10 미만이 되면 차단 상태를 해제한다.
...
| `POST` | `/api/v1/user/chats/{chat}/messages/reports` |
