...
위 경로는 항상 JSON으로 렌더링합니다.

### 4.2 헤더 기반 (선택 JSON)
위 경로가 아니더라도, 클라이언트가 Accept: application/json을 보내면 JSON으로 응답합니다.

React에서 fetch/axios 호출 시 Accept: application/json을 명시하면
/admin/* 페이지 내부에서도 “데이터 호출”에 한해 JSON 에러 포맷을 사용할 수 있습니다.
단, 페이지 네비게이션 자체(/admin/*)는 Inertia 기본 흐름을 유지합니다.
...
- 화면 전환이 깨지거나(JSON만 출력), 폼 오류 표시 흐름이 꼬임
- 로그인/권한 오류가 redirect가 아니라 JSON으로 반환되어 UX가 어색해짐

결론: API는 JSON 통일 / 페이지는 Inertia 기본 흐름 유지가 안정적입니다.
...
본 프로젝트는 예외를 아래 규칙으로 ErrorCode와 HTTP status에 매핑합니다.
...
| `CustomException` | ErrorCode 기준 | ErrorCode 기준 | 도메인/비즈니스 에러 |
| `QueryException` | 500 | `DB_ERROR` | 운영환경에서는 상세 노출 금지 |
...
예외적으로, 관리자 API이고 APP_DEBUG=true인 경우에만 제한적으로 details를 포함할 수 있습니다.
...
## 11. 운영 팁
- 클라이언트(앱/관리자 프론트)는 에러 발생 시 traceId를 함께 로그/리포트에 남길 것
- 서버 로그는 traceId로 검색 가능한 형태를 유지할 것
