...
| 저장값 | 표시명 | 기준 |
|---|---|---|
| `ACTIVE` | 계정 생성 가능 | 미사용·미취소이며 만료 전 |
| `USED` | 계정 생성 완료 | 계정 생성으로 `used_at` 기록 |
| `REVOKED` | 전송 취소 | 재전송 또는 Staff 취소로 `revoked_at` 기록 |
| `EXPIRED` | 기간 만료 | 미사용·미취소이며 만료시각 경과 |

재전송은 아직 유효한 기존 초대만 취소한다. 이미 만료된 이력은 `기간 만료` 상태를 유지한다.
