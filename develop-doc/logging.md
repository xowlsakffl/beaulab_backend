...
`HasAuditLogs`는 비밀번호/remember token/재설정 토큰/인증번호 원문 및 해시를 제외한다. Telescope는 인증·초대·비공개 미디어 경로와 원문 메일/외부 HTTP/Redis/캐시 값 수집을 제한하고, 비밀 컬럼이 포함된 SQL을 저장하지 않는다. 요청/응답의 토큰 필드, 쿠키와 CSRF 헤더도 숨긴다. 업무 검색 조건 로깅 정책과는 별개이며 과거 기록은 자동 삭제하지 않는다. 기존 비밀 값 정리는 [배포 점검](./backend-hardening.md)을 따른다.
